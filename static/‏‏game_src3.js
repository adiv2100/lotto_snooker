...
const CONSTANT_SPEED = false;  // ✅ אחרת ה-power לא יורגש
...
function createBalls() {
  ...
  // --- גריד מיקומים מסודר כמו קודם ---
  const cols = 7;
  const gap = BALL_R * 2 + 6;
  const startX = x0 + (x1 - x0) * 0.60;
  const startY = y0 + 40;

  // --- מספרים 1..37 אבל מעורבבים ---
  const nums = shuffle(Array.from({ length: N_BALLS }, (_, i) => i + 1));

  // --- יוצרים 37 כדורים במיקומים קבועים, עם מספרים רנדומליים ---
  for (let idx = 0; idx < N_BALLS; idx++) {
...
function resolveBallCollision(a, b) {
  ...
  // ✅ רק פגיעה של הכדור הלבן בכדור ממוספר מדליקה firstContact
  if ((a.cue && !b.cue) || (b.cue && !a.cue)) {
    firstContact = true;
  }

  // דוחפים החוצה כדי למנוע "שקיעה"
  const nx = dx / dist;
  const ny = dy / dist;
  ...
  // מהירות יחסית על הנורמל
  const rvx = b.vx - a.vx;
  const rvy = b.vy - a.vy;
  const velAlongNormal = rvx * nx + rvy * ny;

  // אם מתרחקים—לא צריך אימפולס
...
function checkPocket(b) {
  ...
  // ✅ אם זה הכדור הלבן ונפל לפני firstContact → לעצור בלי לפזר
  if (wasCue && !firstContact) {
    running = false;
    // אופציונלי: אפשר גם להציג הודעה/איפוס פה אם תרצה
  }
...
function keepConstantSpeed() {
  ...
  for (const b of balls) {
    ...
    // ✅ לפני הפגיעה הראשונה: רק הכדור הלבן יכול לנוע, כולם נשארים עומדים
    if (!firstContact && !b.cue) continue;

    // ✅ לא "מעירים" כדורים שעומדים – משאירים אותם ב-0
    if (s < ZERO_CUTOFF) {
      b.vx = 0;
      b.vy = 0;
      continue;
